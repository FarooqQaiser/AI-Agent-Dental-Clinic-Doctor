...
function Register() {
  ...
  const submitForm = async (e) => {
    ...
    if (registerObj.password !== registerObj.repeatedPassword)
      return setErrorMessage("Passwods do not match!");
    else {
      setLoading(true);
      console.log("Email in localStorage: ", localStorage.getItem("userEmail"));
      try {
        const response = await fetch(API_URL + "users/signup/send-otp", {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            confirmpassword: registerObj.repeatedPassword,
          }),
        });
      ...
      console.log("User Info: ", registerObj);
      setLoading(false);
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(API_URL + `users/signup/verify-otp`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
            ...
            <div className="mx-auto flex w-full max-w-md flex-col space-y-16">
              <div className="flex flex-col items-center justify-center text-center space-y-2">
                ...
                <div className="flex flex-row text-sm font-medium text-gray-400">
                  <p>We have sent a code to your email {registerObj.email}</p>
                </div>
              </div>
